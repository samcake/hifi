...
void GLShader::makeProgramBindings(ShaderObject& shaderObject) {
    ...
    loc = glGetUniformBlockIndex(glprogram, "cameraCorrectionBuffer");
    if (loc >= 0) {
        glUniformBlockBinding(glprogram, loc, gpu::TRANSFORM_CAMERA_CORRECTION_SLOT);
        shaderObject.transformCameraCorrectionSlot = gpu::TRANSFORM_CAMERA_CORRECTION_SLOT;
    }
...
GLShader* GLShader::compileBackendShader(GLBackend& backend, const Shader& shader) {
...
GLShader* GLShader::compileBackendProgram(GLBackend& backend, const Shader& program) {
